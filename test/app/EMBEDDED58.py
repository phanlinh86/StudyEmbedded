...
class EMBEDDED58(TestInstances):
    LED_GREEN = "D12"
    LED_ORANGE = "D13"
    LED_RED = "D14"
    LED_BLUE = "D15"
    ...
    def run(self):
        ...
        self.mcu.writegpio(self.LED_GREEN, 3)
        self.mcu.writegpio(self.LED_ORANGE, 3)
        self.mcu.writegpio(self.LED_RED, 3)
        self.mcu.writegpio(self.LED_BLUE, 3)
        ...
        try:
            while True:
                ...
                self.blink()    # Blink the LED based on accelerometer values
                # Plot the cube
    ...
    def blink(self):
        # Blink the LED according to the accelerometer values
        if self.roll > 30:
            self.mcu.writegpio(self.LED_GREEN, 1)
            self.mcu.writegpio(self.LED_RED, 0)
        elif self.roll < -30:
            self.mcu.writegpio(self.LED_GREEN, 0)
            self.mcu.writegpio(self.LED_RED, 1)
        else:
            self.mcu.writegpio(self.LED_GREEN, 0)
            self.mcu.writegpio(self.LED_RED, 0)

        if self.pitch > 30:
            self.mcu.writegpio(self.LED_ORANGE, 1)
            self.mcu.writegpio(self.LED_BLUE, 0)
        elif self.pitch < -30:
            self.mcu.writegpio(self.LED_ORANGE, 0)
            self.mcu.writegpio(self.LED_BLUE, 1)
        else:
            self.mcu.writegpio(self.LED_ORANGE, 0)
            self.mcu.writegpio(self.LED_BLUE, 0)
...
if __name__ == "__main__":
    blink_led = EMBEDDED58(port='COM9',
                           baudrate=500000,
                           center=[0, 0, 0],
                           width=2,
                           height=1,
